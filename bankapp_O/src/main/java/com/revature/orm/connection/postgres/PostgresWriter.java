...
public class PostgresWriter<T> implements StatementWriter<T> {
	...
	public PreparedStatement insert(T obj, Connection conn) throws SQLException {
		...
		PreparedStatement st = conn.prepareStatement(sql.toString(), keys);
		return st;
	}
		...
		if (field.matches("(\\w+[.]\\w+[.]?)+")) {
			...
			try {
				...
				if (fieldExists(parsedObj.getOriginalType(), fields[fields.length - 2])) {
					...
					Object tableField = parsedObj.getOriginalType().getMethod(methodName).invoke(parsedObj.getOriginalType().newInstance());
					fieldObj = new ParsedObject(tableField.getClass());
				}
				String subField = fieldObj.getColumns().get(fields[fields.length - 1]);
				sql.append(" where " + fieldObj.getTableName() + "." + subField + "=?");
			} catch (NoSuchFieldException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException | NoSuchMethodException | InstantiationException e) {
				e.printStackTrace();
			}
		} else {
	...
	public PreparedStatement delete(T obj, Connection conn) throws SQLException {
		StringBuilder sql = new StringBuilder("delete from " + parsedObj.getTableName() + " where " + parsedObj.getPrimaryKeyColumn() + "=?");
		PreparedStatement st = conn.prepareStatement(sql.toString());
		return st;
	}
	...
	private Object selectColumnsList() {
		...
		for (String column : parsedObj.getColumns().values()) {
			sql.append(parsedObj.getTableName() + "." + column + " as " + parsedObj.getTableName() + "_" + column + ",");
		}
		for (ParsedObject innerObj : parsedObj.getRelationships().keySet()) {
			for (String column : innerObj.getColumns().values()) {
				sql.append(innerObj.getTableName() + "." + column + " as " + innerObj.getTableName() + "_" + column + ",");
			}
		...
		for (ParsedObject innerObj : parsedObj.getRelationships().keySet()) {
			...
			case MANY_TO_ONE:
				sql.append(" left join " + innerObj.getTableName() + " on " + parsedObj.getTableName() + "."
						+ info.getOwnerJoinColumn() + " = " + innerObj.getTableName() + "." + innerObj.getPrimaryKeyColumn());
				break;
			case ONE_TO_MANY:
				sql.append(" left join " + innerObj.getTableName() + " on " + parsedObj.getTableName() + "."
				...
				break;
			case MANY_TO_MANY:
				sql.append(" left join " + info.getJoinTable() + " on " + parsedObj.getTableName() + "." + parsedObj.getPrimaryKeyColumn()
				+ " = " + info.getJoinTable() + "." + info.getOwnerJoinColumn() + " join " + innerObj.getTableName() + " on " + "."
